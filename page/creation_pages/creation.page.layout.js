      ...
      data_config: [
        {
          data_array: new Array(24)
            .fill(0)
            .map((d, index) => formatNumberForPicker(index)),
          support_loop: true,
          init_val_index: hh,
          connector: ":",
...
function renderPage(pageName = "", confirmCallback) {
  ...
  const backgroundContainer = createWidget(
    widget.VIEW_CONTAINER,
    COMMON_LAYOUT.CONTAINERS.fullScreenContainer()
  );
  backgroundContainer.createWidget(
    widget.WIDGET_PICKER,
    CONTROLS.timePicker(
      hh,
      mm,
      ss,
      (picker, event_type, column, valueIndex) => {
        if (event_type === 1) {
          switch (column) {
            case 0:
              hh = valueIndex;
  ...
  const header = backgroundContainer.createWidget(
    widget.TEXT,
    COMMON_LAYOUT.HEADERS.header(pageName)
  );
}
